	...
	public class Bulkapi {
		...
		String[] sObjectList = objectName.split(",");
		private JobInfo jobinfo;
		String soqlQuery;
		BulkConnection connection;
		PartnerConnection soapPartnerConnection;
		String Metainfo;


		 public static void main(String[] args) throws AsyncApiException, ConnectionException, IOException, ConfigurationException, JSONException {
			 Bulkapi bka = new Bulkapi();
			        ...
			        // bka.sampleUpload("Account", "YOURSFUSERNAME", "yoursfpasswordandsectoken", "mySampleData.csv");
			          bka.bulkExport("pateldiptig@resourceful-wolf-agedhh.com", "Mahant1933PZYcSc31ia0rDnw3uTr9QTcx");
			     ...
			     */
			    public void sampleUpload(String sobjectType, String userName, String password, String sampleFileName) throws AsyncApiException, ConnectionException, IOException {
			        BulkConnection connection = getBulkConnection(userName, password);
			        System.out.println("Connection value is ==>"+connection);
			        JobInfo job = createJob(sobjectType, connection);
			        //System.out.println("JobInfo value is ==>"+job);
			    ...
			    /**
			     * Creates a Bulk API job and to download data from Salesforce.
			     */  
			    ...
			    //    job.setOperation(operation);
			       ...
			       // job.setOperation(OperationEnum.insert);
			        ...
			        System.out.println("Job info is--->"+job);
			        return job;
			    }
			    ...
			    private BulkConnection getBulkConnection(String userName, String password)
			        ...
			        soapPartnerConnection =   new PartnerConnection(partnerConfig);
			...
			private  BatchInfo createBatchesFromQuery(String query,JobInfo jobinfo,BulkConnection connection) throws AsyncApiException,IOException 
			 {
				System.out.println("Query is "+query+" "+jobinfo);
			 ...
			 private String createQueryFromInput(String sObject) throws ConfigurationException,ConnectionException,JSONException
			 {
				 QueryGenerator SFQGen = new QueryGenerator(soapPartnerConnection,sObject);
				 Metainfo = SFQGen.getMetaString();
				 System.out.println("Columns are " + Metainfo);
			     return SFQGen.getQuery();
				 
	       	 }
			 
			 public  void bulkExport(String userName,String password) throws ConnectionException, AsyncApiException, ConfigurationException, IOException, JSONException
			 {
				 List<BatchInfo> ls = null;
				 BulkConnection connection = getBulkConnection(userName,password);
				 System.out.println("Connection in main is==>"+connection);
				 JobInfo jobinfo = createJob(objectName,connection);
			     createBatchesFromQuery(createQueryFromInput(objectName), jobinfo,connection);
				 closeJob(connection,jobinfo.getId());
				 BatchInfoList  batchinfo = connection.getBatchInfoList(jobinfo.getId());
		      }
